...
function RateLimit(options) {
  console.log('RateLimit: OPTIONS BEFORE DEFAULTS', options);
  ...
  console.log('RateLimit: OPTIONS AFTER DEFAULTS', options);
  ...
  function rateLimit(req, res, next) {
    ...
    options.store.incr(key, function(err, current, resetTime) {
      ...
      console.log('RateLimit: max result', maxResult, options.max);
      ...
      Promise.resolve(maxResult)
        ...
        .then(max => {
          ...
          if (max && current === max + 1) {
            console.log('RateLimit: OVER LIMIT', max);
